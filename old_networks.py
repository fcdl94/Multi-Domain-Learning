...
def conv3x3(in_planes, out_planes, stride=1, quantized=0, masked=False):
    if masked:
        ...
        return custom_layers.QuantizedConv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False,
                                             masks=1)
        ...
        if first:
            if masked:
                self.downsample = nn.Sequential(
                    custom_layers.MaskedConv2d(inplanes, planes, kernel_size=1, stride=stride, bias=False),
                    nn.BatchNorm2d(planes))
            elif quantized:
                self.downsample = nn.Sequential(
                    custom_layers.QuantizedConv2d(inplanes, planes, kernel_size=1, stride=stride, bias=False, masks=1),
                    nn.BatchNorm2d(planes))
            else:
...
class ResNet28(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride, masked=False, quantized=0, bn=False):
        ...
        for i in range(len(strides)):
            layers.append(
                block(self.inplane, planes, strides[i], first=(i == 0), masked=masked, quantized=quantized, bn=bn))
            self.inplane = planes
